...
function fireRouterEvent(type, detail) {
  window.dispatchEvent(new CustomEvent(`vaadin-router-${type}`, {
    detail
  }));
}
...
class Resolver {
  ...
  resolve(pathnameOrContext) {
    ...
    const match = matchRoute(this.root, context.pathname.substr(this.baseUrl.length));
...
class Router extends Resolver {
  ...
  __resolveRoute(context) {
    ...
    if (route.bundle) {
      callbacks = callbacks.then(() => loadBundle(route.bundle)).catch(() => {
  ...
  __fullyResolveChain(originalContext, currentContext = originalContext) {
    return this.__amendWithResolutionResult(currentContext).then(amendedContext => {
      ...
      return amendedContext.next().then(nextContext => {
        ...
        return nextContext ? this.__fullyResolveChain(initialContext, nextContext) : this.__amendWithOnBeforeCallbacks(initialContext);
      });
    });
  }
